                                  ...
                                  FEDRawData& rawdata, const EcalElectronicsMapping* TheMapping)
{
  ...
  const int maxTccPerDcc = 4;

  EcalDCCHeaderBlock dccHeader = base_->GetDCCHeader(iDCC);
  std::vector<short> bx  = dccHeader.getTCCBx();
  std::vector<short> lv1 = dccHeader.getTCCLv1();
  bx.resize(maxTccPerDcc);
  lv1.resize(maxTccPerDcc);
  ...
  if (IsEndCap) {
    ...
    int pair = (TCCid -1 ) % 2;	// the TCCid is written to the RawData.
    int inner = ( detid.ietaAbs() >= 22) ? 1 : 0;
    itcc_block = 2 * pair - inner + 2;
  ...
  if ((int)rawdata.size() != HEADER_SIZE) {
    ...
    for (int iblock=0; iblock < NTCC; iblock++) {     // do this once per fed in EB, four times in EE
      ...
      ppData[8*fe_index+1] = 0x10; //value undefined in spec, seen at this value in  data
      ppData[8*fe_index+2] =  bx[iblock] & 0xFF;      // bx takes bits 0-11: 0-7+8-11
      ppData[8*fe_index+3] = (bx[iblock] & 0xF00)>>8;
      ppData[8*fe_index+3] |= 0x60;
      ppData[8*fe_index+4] =  lv1[iblock] & 0xFF;      // same game done for lv1, which takes bits 0-11: 0-7+8-11
      ppData[8*fe_index+5] = (lv1[iblock] & 0xF00)>>8; // lv1
  ...
  int gap = 0;
  if(itcc_block % 2 == 0 && iTT > 8) gap = 2*8;       // outer TCCs have two blank rows between TP 8 and 9
  ...
  int sfgvb = trigprim.sFGVB();

  if (debug_ && (ttflag != 0)) {
    cout << "in TCCBlock : this tower has a non zero flag" << endl;
    cout << "Fedid  iTT  flag " << dec << FEDid << " " << iTT << " " << "0x" << hex << ttflag << endl;
  }
  pData[8*FE_index + ival*2 + gap] = et & 0xFF;                   // ival is location inside a TP row; varies between 0-3; tp goes in bits 0-7
  pData[8*FE_index + ival*2+1 + gap] = (sfgvb <<4) | (ttflag<<1) | (fg&0x1);    // fg follows in bit 8; ttfg is in bits 9-11 ; sfgvb in bit 12
  if (IsEndCap) {
  ...
  if (ival %2 == 1){
    ...
    pData[8*FE_index + ival*2+1 + gap] |= 0x60;
  }
