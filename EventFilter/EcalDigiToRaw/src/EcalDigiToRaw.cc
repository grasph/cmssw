  ...
  pDccHeaders_(0)
{
  ...
  labelDCCHeader_ = iConfig.getParameter<edm::InputTag>("labelDCCHeader");
...
EcalDigiToRaw::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(iEvent.getByLabel(labelDCCHeader_, hDccHeaders)){
    pDccHeaders_ = hDccHeaders.product();
  } else{
    if(debug_){
      std::cerr << __FILE__ << ":" << __LINE__
                << ": " << "DCC header collections with label "
                << labelDCCHeader_ << " was not found!"
                << std::endl;
    }
    pDccHeaders_ = 0;
  }
