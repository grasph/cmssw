...

{
  ...
  if (fe != FEorder.end()) {
    ...
    std::vector<short> bx = dccHeader.getFEBxs();
    std::vector<short> lv1 = dccHeader.getFELv1();
    int thisbx  = (unsigned) FE_order < bx.size()  ? bx[FE_order]  : 0;
    int thislv1 = (unsigned) FE_order < lv1.size() ? lv1[FE_order] : 0;
    pData[8*FE_index] = iFE & 0xFF;
    pData[8*FE_index+1] = (nsamples & 0x7F);
    pData[8*FE_index+2] = thisbx & 0xFF;
    pData[8*FE_index+3] = (thisbx >>8) & 0x0F;
    //    pData[8*FE_index+3] |= 0xa0;
    pData[8*FE_index+3] |= 0xC0;
    pData[8*FE_index+4] = thislv1 & 0xFF;
    pData[8*FE_index+5] = (thislv1 >>8) & 0x0F;
...
void TowerBlockFormatter::EndEvent(FEDRawDataCollection* productRawData) {
  ...
  // -- Need to reorder the FE's in the TowerBlock. They come in the right
...
#else //Let's simplify the code:
    // iDAQ_header = 0;
    // iDCC_header = 1;
    // iTCCBlock_header = 9;
    // iSRBlock_header = ((idcc - 1) % 45) < 9 ? 45 : 27);
    // iTowerBlock_header = iSRBlock_header + 6;
    int iDAQ_trailer = length - 1;
    int iTowerBlock_header = length - 1;
    while(iTowerBlock_header > 1 && (words[iTowerBlock_header-1] >>62) == 0x3) --iTowerBlock_header;
#endif
    ...
    while (iTowerBlock_header < iDAQ_trailer) {
      ...
      if(nlines==0){
        std::cerr << __FILE__ << ":" << __LINE__ << ": "
                  << "Invalid tower block length for dcc " << idcc << ", CCU ID " << fe << "!" << std::endl;
        break;
      }
      if(fe > 0 && fe < 69){
